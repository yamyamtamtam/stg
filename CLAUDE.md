...
- 描画: drawBG(デフォルトは01レイン+グリッチ。シナリオ定義に `bgTheme:"sea"/"forest"/"salon"`
  があるとプレイ中はシナリオ固有のピクセルアート背景+ボス戦専用背景にクロスフェード。
  レイヤーは初回のみプリレンダし毎フレームはdrawImage数回に抑える) →
  敵(e.sprite優先) → ボス(浮遊カード) → 自機 → 弾 →
...
- ASIデモ: `demoDodge()` = ビームサーチ式の経路探索AI(**無敵チート無し**。`playerHit()`は
  デモ中も普通に効く)。弾を等速直線で先読みし、生き残れる経路の一手目だけ実行して再探索
  (余裕がある間は2フレームに1回に間引き。ホットパスは使い回しtyped arrayでGC/Mapコストを
  排除し実機60fpsに収める)。`demoSafeSpot()`(2D安全マップ)が長期の退避先を、ルート定義の
  `demoBeltY(boss)`が推奨滞空高度を与える。探索が「全経路死亡」と判断した緊急時のみ自動で
  ボムを使う(妥協ライン=ボム使用・3死以内クリア)。葡萄弾幕は「生存回廊」(ゆっくり漂う
  回廊柱を滞空帯で横切る弾道を発射時に間引く仕組み)で理論上回避不能な瞬間を構造的に排除。
  洗濯機は発射源固定・元密度のまま(移動させると弾が薄まる+処理落ちするため撤回済み)
...
- 敵弾の当たり判定は円(b.r)だが、被弾判定は b.r の85%(見た目より微妙に小さい=プレイヤー有利。
  グレイズ範囲は見た目のまま)。月弾/文字弾は見た目の方が大きい方向で統一
