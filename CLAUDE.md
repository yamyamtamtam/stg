...
- ASIデモ: `demoDodge()` = 9方向それぞれについて数フレーム先の弾との接近度を採点し、
  最も安全な方向へ低速移動するシンプルな回避AI(ホームポジション=ボス直下への弱い引力で
  射線も維持)。デモ中の自機は `playerHit()` 先頭の early return で被弾しない(演出優先)
...
1. `js/scenarios/scenarioN_xxx.js` を新規作成。既存3ファイル(シナリオ1〜3)が実例で、契約は:
   ...
   ```
   分岐のあるシナリオは上記の代わりに `routes:[{name, sub, ...上記と同じ契約}, ...]` を
