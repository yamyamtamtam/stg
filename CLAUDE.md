...
- ASIデモ: `demoDodge()` = ビームサーチ式の経路探索AI(**無敵チート無し**。`playerHit()`は
  ...
  ボムを使う(妥協ライン=ボム使用・3死以内クリア。葡萄は理論上回避不能な瞬間が数百フレームに
  一度あることが全数探索で証明済みで、毎回この上限近くまで使い切るのが正常動作)。
  生存回廊(通り道の保証)や発射源の移動は「自機付近に隙間ができて簡単になる/弾が薄まる」
  ためどちらも導入後に撤回済み。弾幕を簡単にする方向の変更は入れないこと
