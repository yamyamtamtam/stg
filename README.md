...
- シナリオ選択制(タイトル後に選択):
  ...
  - 「シンギュラリティ」: 道中なし・開幕即ボス会話 → 棗みそのの2フェーズ連戦
    - フェーズ0「葡萄」: 大玉高密度ランダム散布型弾幕(虫姫さまウルトラ真ボス・アキ最終弾幕の近似再現)
    - 中間会話を挟んでフェーズ1: 二層逆回転型の回転撃ち弾幕(スペカ名なし)
    - 難易度はこのシナリオ専用の二択「死ぬがよい(人間用)」「君ならできるよ（笑）(AI用)」。人間用は弾速のみ大幅に遅い
