...
| Z | ショット / 決定・会話送り |
| X | 陽符(ボム) |
| Shift | 低速移動(スマホオプション前方集中)。当たり判定マーカーは常時表示 |
| Esc | ポーズ |
...
タブレットは画面レイアウトが縦横で変わる: 縦向きはスマホと同じ全画面レイアウト、
横向きはPCと同じサイドパネル付きレイアウト(操作方法はどちらもタッチのまま)。
...
- シナリオ選択制(タイトル後に選択):
  ...
  - 「シンギュラリティ」: 選択後にルート分岐「死ぬがよい」/「君ならできるよ（笑）」。各ルートは
    さらに人間用/AI用/ASIデモプレイの3択を持つ独立した単発ボス戦(道中なし・開幕即ボス会話)
    - 「死ぬがよい」ルート = 二層逆回転型の回転撃ち弾幕(スペカ名なし)
    - 「君ならできるよ（笑）」ルート = 大玉高密度ランダム散布型弾幕(虫姫さまウルトラ
      真ボス・アキ最終弾幕の近似再現)。発射源(敵機)が移動しながら下向きに散布し、
      ボスより上の安置へ逃げると超高速の自機狙い弾が飛んでくる
    - 各ルートとも人間用は弾速・密度ひかえめ(画面上のサブタイトル表示はなし)
    - 「君ならできるよ（笑）」の弾幕には「ゆっくり漂う見えない通り道(生存回廊)」が構造的に
      保証されている。ASIデモプレイは無敵チート無しの回避AI(経路探索)が実際に避けて戦い、
      避け切れない緊急時のみボムで凌ぐ
- ボス戦前に会話パート、グレイズ・スペルカードカットインなど東方風システム一式
- SEはWeb Audio APIによる8bit風プロシージャル合成(音源ファイル不要)、BGMはタイトル〜チュートリアル/道中/ボス戦(共通)/シンギュラリティ専用の4曲を切替再生
